...
router.post("/add", async (req, res) => {
  try {
    const { deviceId, itemId, itemName, quantity } = req.body;
    const qty = Number(quantity);

    if (!deviceId || (!itemId && !itemName) || !Number.isInteger(qty) || qty <= 0) {
      return res.status(400).json({ error: "Invalid cart item" });
    }

    let selected = itemId ? await MenuItem.findById(itemId) : null;

    if (!selected && itemName) {
      selected = await MenuItem.findOne({ name: itemName });
    }
